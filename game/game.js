...
function isWalkable(x,y) {
  ...
  if (tile?.walk == false) {
    return false;
  } else if (check.some(([key, ent]) => defaultnonmove.includes(ent.type)) && tile?.walk != true) {
    return false;
  }
  return true;
}
...
function pushent(x, y, dx, dy) {
  const target = getEntityAt(x, y); //TODO change to get entities
  ...
  moveEntity(target.id, dx, dy);
  return true;
}

function getEntitiesAt(x, y) { //useless
